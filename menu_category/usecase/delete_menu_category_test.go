...
func TestDeleteMenuCategory_normalCase(t *testing.T) {
	...
	currentUnixTime := time.Now().UTC().UnixMicro()
...
func TestDeleteMenuCategory_errorWhenSomethingWrongWithMarshalingOrDatabase(t *testing.T) {
	...
	mockMenuCategoryRepository.Mock.On("DeleteMenuCategory", ctx, normalUpsertRequest.UUID).Return(nil, http.StatusInternalServerError, errors.New("something wrong"))
